...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    if (b === 0) {
      throw new Error('Infinite solutions, as any x satisfies the equation');
    } else {
      throw new Error('No solution, as equation is inconsistent');
    }
  }
  return -b / a;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.abs([x1, x2, x3].reduce((sum, num) => sum + num, 0)).toFixed(1);
}
